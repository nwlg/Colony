...
public class ListarTaxonomiasControllerImpl extends ListarTaxonomiasController
{
    ...
    throws Exception
    {
		...
		try{
		Collection vdex=Arrays.asList(this.getSrvEstructurasEducativasService().listarBackups());
	...
	throws Exception 
	{
		   ...
		   && form.getTipo()!=null && !form.getTipo().equals(""))
		{
			...
			try{
				if(form.getTipo().equals("TAX"))
					dh = this.getSrvEstructurasEducativasService().exportarVdex(form.getNombre(), TipoVdex.TAXONOMIA);
				else
					dh = this.getSrvEstructurasEducativasService().exportarVdex(form.getNombre(), TipoVdex.TESAUROS_BACK);
				if(dh!=null)
				{
