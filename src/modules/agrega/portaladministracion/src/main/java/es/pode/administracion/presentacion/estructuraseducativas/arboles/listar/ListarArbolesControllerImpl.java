...
public class ListarArbolesControllerImpl extends ListarArbolesController
{

	private static Logger logger=Logger.getLogger(ListarArbolesControllerImpl.class);


    public final void obtenerArboles(
    		ActionMapping mapping, 
    		ObtenerArbolesForm form, 
    ...
    throws Exception
    {
		...
		try{
			...
			logger.error("error en ListarArbolesControllerImpl.obtenerArboles: obteniendo idiomas ");
		}
	    
		try{
		Collection vdex=Arrays.asList(this.getSrvEstructurasEducativasService().listarArbolCurricular());
		form.setArboles(vdex);
		}catch (Exception e) {
			logger.error("error en ListarArbolesControllerImpl.obtenerArboles: obteniendo vdex ");
		}
    ...
    public final void validarSeleccionados(
    		ActionMapping mapping, 
...
//		    		throw new ValidatorException("{catalogadorAvanzado.exportar.error.fichero}");
			...
			logger.debug("error al intentar exportar el arbol curricular ");
		}
