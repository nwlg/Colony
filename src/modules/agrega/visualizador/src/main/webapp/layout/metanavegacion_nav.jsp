...
{
 parent.location='<html:rewrite action="/Visualizar/VisualizarDatosNavSecuenciaBoton"/>?identificador=' + identificador + '&usuario=' + usuario + '&contador=' + contador;
}
</script>
...
<span class="flechas">
     <c:if test="${visualizadorSession.contador > 0 }" >
		<c:if test="${visualizadorSession.btnIzquierdo }">
		<bean:define id="anterior"><bean:message key="cabecera.VerAnterior"/></bean:define>
		<html:link href="javascript:clickBoton('${visualizadorSession.contador -1 }' , '${visualizadorSession.identificador}', '${visualizadorSession.usuario}');" title="${anterior }" styleClass="f_left">&nbsp;&nbsp;<span><bean:message key="cabecera.InformeAnterior"/></span></html:link>
		</c:if>
	 </c:if>
	 
	 <c:set var="longitud" value="${fn:length(visualizadorSession.itemsFlow) -1}" />
	 <c:if test="${visualizadorSession.contador < longitud}">
		<c:if test="${visualizadorSession.btnDerecho }">
		<bean:define id="siguiente"><bean:message key="cabecera.VerSiguiente"/></bean:define>
			<html:link href="javascript:clickBoton('${visualizadorSession.contador +1}', '${visualizadorSession.identificador}', '${visualizadorSession.usuario}');" title="${siguiente }" styleClass="f_right">&nbsp;&nbsp;<span><bean:message key="cabecera.InformeSiguiente"/></span></html:link>
		</c:if>
	</c:if>
</span>	
</div>
