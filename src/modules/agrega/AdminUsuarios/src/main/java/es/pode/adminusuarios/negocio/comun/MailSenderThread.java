...
public class MailSenderThread extends Thread {
	...
	public MailSenderThread(String to, String sub, StringBuffer bodyEmail,
			String fromEmail) {
		toAddress = to;
		subject = sub;
		body = bodyEmail;
	...
	public void send(String toAddress, String subject, StringBuffer body,
			String from) {

		log("Send email");
		log("toAddress " + toAddress);
log("MAILSENDERTHREAD");
		...
		try {
			...
			session.setDebug(new Boolean(this.getPropertyValue("debug"))
					.booleanValue());
			...
			isDone = true;
			log("finaliza la ejecucion del hilo de envio de correo");
		}
	...
	public void run() { // Sobrecargando al metodo run
		...
		while (!isDone) {
			...
			this.send(toAddress, subject, body, from);
